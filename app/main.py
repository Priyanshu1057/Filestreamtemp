...
async def stream_file(request: Request, short_code: str):
    ...
    # Every session resolves the media itself: file references are per-account.
    pairs = await session_manager.resolve_media(
        file_data['chat_id'], file_data['message_id']
    )

    if not pairs:
        logger.error(
            f"No session could read {short_code} "
            f"(chat {file_data['chat_id']}, msg {file_data['message_id']})"
        )
        raise HTTPException(
            status_code=404,
