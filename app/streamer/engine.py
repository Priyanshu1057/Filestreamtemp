...
async def ultra_high_speed_streamer(pairs: list, start: int, end: int, chunk_size: int = 1024 * 1024):
    """
    Refactored ultra-high-speed multi-session streamer.

    `pairs` is a list of (client, file) tuples. Each client MUST be paired with a
    file reference it fetched itself — Telegram file references are per-account,
    and mixing them causes "no access to file" errors.
    """
    import asyncio
    ...
    if session_count == 1:
        ...
        client, file = pairs[0]
        try:
            async for chunk in client.iter_download(file, offset=start, limit=total_to_send, request_size=chunk_size):
    ...
    async def fetch_worker(client: TelegramClient, file, worker_id: int):
        while True:
            try:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.warning(f"Worker {worker_id} chunk {offset} failed: {e}")
    ...
    workers = [
        asyncio.create_task(fetch_worker(*pairs[i % session_count], i))
...
async def get_streaming_response(pairs: list, file_size: int, filename: str, mime_type: str, request: Request):
    ...
    return StreamingResponse(
        ultra_high_speed_streamer(pairs, start, end),
