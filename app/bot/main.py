...
def register_handlers(bot):
    ...
    async def media_handler(event):
        ...
        # IMPORTANT: the incoming message lives in the bot's PRIVATE chat with the
        # user. User sessions (SESSIONS=...) cannot read that chat, which is why
        # streaming fails with "no access to file" as soon as sessions are added.
        # Copy the media into the storage channel that every session can read, and
        # point the link at that copy instead.
        store_chat_id = event.chat_id
        store_message_id = event.id

        if settings.CHANNEL_ID:
            try:
                stored = await bot.send_file(
                    settings.CHANNEL_ID,
                    event.message.media,
                    caption=f"`{file_name}`\nUploader: `{event.sender_id}`\nCode: `{short_code}`",
                )
                if isinstance(stored, list):
                    stored = stored[0]
                store_chat_id = settings.CHANNEL_ID
                store_message_id = stored.id
            except Exception as e:
                logger.error(f"Could not copy media to storage channel: {e}")
                if session_manager.has_user_sessions():
                    return await event.reply(
                        "⚠️ **Storage channel unavailable.**\n\n"
                        "The bot could not save your file to the storage channel, so a "
                        "link cannot be generated. Please contact the admin."
                    )
        elif session_manager.has_user_sessions():
            logger.error("SESSIONS are configured but CHANNEL_ID is not set.")
            return await event.reply(
                "⚠️ **Bot is misconfigured.**\n\n"
                "A storage channel (CHANNEL_ID) is required. Please contact the admin."
            )
